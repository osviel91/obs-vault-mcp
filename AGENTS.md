...
- On a writer-triggered request, `vault-sync` waits 5 s and then runs `rclone sync` twice (5 s gap between passes). This is intentional: NAS WebDAV servers can take a few seconds to propagate new directory listings after a write/move, and a single sync would race the propagation and report "completed" without transferring the new files. Scheduled syncs run a single pass. Do not remove the settle + double sync without understanding the read-after-write race.
