...
- On a writer-triggered request, `vault-sync` waits 240 s and then runs `rclone sync` twice (60 s gap between passes). This is intentional: the NAS WebDAV used here takes several minutes to propagate new directory listings after a write/move, and a single sync would race the propagation and report "completed" without transferring the new files. Scheduled syncs run a single pass. Do not reduce the settle or remove the double sync without understanding this read-after-write race.
